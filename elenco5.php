...
if (tiporic == "di"){
	...
	if (query != "") {
		...
		if (modello != "")
			query = query + " WHERE Modello like '%" + modello + "%'";
	}
	...
	if (query != "") {
		...
		if (annoda != "")
			query = query + " WHERE AnnoImmatricolazione >= '" + annoda + "'";
	}
	...
	if (query != "") {
		...
		if (annoa != "")
			query = query + " WHERE AnnoImmatricolazione <= '" + annoa + "'";
	}
	...
	if (query != "") {
		...
		if (prezzoda != ""){
			query = query + " WHERE Prezzo >= '" + prezzoda + "'";
		}
	}
	...
	if (query != "") {
		...
		if (prezzoa != ""){
			query = query + " WHERE Prezzo <= '" + prezzoa + "'";
		}
	}
	...
	if (query != "") {
		...
		if (chiloda != "") {
			query = query + " WHERE Chilometraggio >= '" + chiloda + "'";
		}
	}
	...
	if (query != "") {
		...
		if (chiloa != "") {
			query = query + " WHERE Chilometraggio <= '" + chiloa + "'";
		}
	}
	...
	if (query != "") {
		...
		if (idcarb != "") 
			query= query + " WHERE idCarburante='" + idcarb + "'";
	}
	...
	if (query != "") {
		...
		if (versione != "") 
			query= query + " WHERE Versione like '%" + versione + "%'";
	}
	...
	if (query != "") {
		...
		if (cambio != "") 
			query= query + " WHERE Cambio like '%" + cambio + "%'";
	}
	...
	if (query != "") {
		...
		if (idcolore != "") 
			query= query + " WHERE idColoreEsterno='" + idcolore + "'";
	}
	...
	if (query != "") {
		...
		if (pkwda != "") {
			query = query + " WHERE PotenzaKW >= '" + pkwda + "'";
		}
	}
	...
	if (query != "") {
		...
		if (pkwa != "") {
			query = query + " WHERE PotenzaKW <= '" + pkwa + "'";
		}
	}
	...
	if (query != "") {
		...
		if (pcvda != "") {
			query = query + " WHERE PotenzaCV >= '" + pcvda + "'";
		}
	}
	...
	if (query != "") {
		...
		if (pcva != "") {
			query = query + " WHERE PotenzaCV <= '" + pcva + "'";
		}
	}
	...
	if (query != "") {
		...
		if (airbag != "") 
			query= query + " WHERE Airbag='" + airbag + "'";
	}
	...
	if (query != "") {
		...
		if (Antifurto != "") 
			query= query + " WHERE Antifurto='" + Antifurto + "'";
	}
	...
	if (query != "") {
		...
		if (ccentr != "") 
			query= query + " WHERE ChiusuraCentralizzata='" + ccentr + "'";
	}
	...
	if (query != "") {
		...
		if (immobilizer != "") 
			query= query + " WHERE Immobilizer='" + immobilizer + "'";
	}
	...
	if (query != "") {
		...
		if (freni != "") 
			query= query + " WHERE FreniADisco='" + freni + "'";
	}
	...
	if (query != "") {
		...
		if (aele != "") 
			query= query + " WHERE AlzacristalliElettrici='" + aele + "'";
	}
	...
	if (query != "") {
		...
		if (navigatore != "") 
			query= query + " WHERE NavigatoreSatellitare='" + navigatore + "'";
	}
	...
	if (query != "") {
		...
		if (radio != "") 
			query= query + " WHERE RadioCD='" + radio + "'";
	}
	...
	if (query != "") {
		...
		if (pdc != "") 
			query= query + " WHERE ParkDistControl='" + pdc + "'";
	}
	...
	if (query != "") {
		...
		if (sedili != "") 
			query= query + " WHERE SediliRiscaldati='" + sedili + "'";
	}
	...
	if (query != "") {
		...
		if (Servosterzo != "") 
			query= query + " WHERE Servosterzo='" + Servosterzo + "'";
	}
	...
	if (query != "") {
		...
		if (volante != "") 
			query= query + " WHERE VolanteMultifunzione='" + volante + "'";
	}
	...
	if (query != "") {
		...
		if (Handicap != "") 
			query= query + " WHERE Handicap='" + Handicap + "'";
	}
	...
	if (query != "") {
		...
		if (cerchi != "") 
			query= query + " WHERE CerchiInLega='" + cerchi + "'";
	}
	...
	if (query != "") {
		...
		if (gancio != "") 
			query= query + " WHERE GancioTraino='" + gancio + "'";
	}
	...
	if (query != "") {
		...
		if (ssportivi != "") 
			query= query + " WHERE SediliSportivi='" + ssportivi + "'";
	}
	...
	if (query != "") {
		...
		if (pacchi != "") 
			query= query + " WHERE Portapacchi='" + pacchi + "'";
	}

	var citta = "<?php echo $_REQUEST["citta"]?>";
	var distanza = "<?php echo $_REQUEST["distanza"]?>";
	
	if (distanza != "") 
		query= query + " HAVING distanza<="+distanza;
	else{
		if (query != "") {
			if (citta != "") 
			...
			if (citta != "") 
				query= query + " WHERE comune like '%" + citta + "%'";
		}
	}
...
function Elenco(query, start, limit, tipoRic, page, ordine, citta, distanza){
	if (ordine == "")
	...
	$.ajax({
		...
		data: "start=" + start + "&limit=" + limit + "&tiporic=" + tiporic + "&query=" + escape(query) + "&page=" + page + "&ordine=" + ordine + "&citta=" + escape(citta) + "&distanza=" + distanza + "&action=elencoauto5",
		async: false,
	...
	$.ajax({
		type: "POST",
		url: "ajax/gestione.php",
		data: "start=" + start + "&limit=" + limit + "&tiporic=" + tiporic + "&query=" + escape(query) + "&page=" + page + "&ordine=" + ordine + "&citta=" + escape(citta) + "&distanza=" + distanza + "&action=elencopag5",
		async: false,
		success: function(msg) 
		{
			//alert(msg);
				...
				<div class="list_manage">
					<div class="inner clearfix">
						...
						<span class="manage_title">Ordina Per:</span>
							<select class="select_styled white_select" name="sort_list" id="sort_list" onchange='Elenco(query, start, limit, tiporic, page, (this.value), citta, distanza)'>
				...
				<div class="widget-container widget_adv_filter">
					...
					<script type="text/javascript" >
							...
							jQuery(document).ready(function($) {	
								...
								Elenco(query, start, limit, tiporic, page, ordine, citta, distanza);
