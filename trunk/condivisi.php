...
if(tiporic=="1") {
	query =" condividiveicolo=1";
}

function Elenco(query, start, limit, tipoRic, page, ordine){
	if (ordine == "")
			query = query + " ORDER BY PrezzoConcessionari ASC";
		else if (ordine == 1)
			query = query + " ORDER BY PrezzoConcessionari ASC";
		else if (ordine == 2)
			query = query + " ORDER BY PrezzoConcessionari DESC";
		else if (ordine == 3)
	...
	$.ajax({
		...
		data: "start=" + start + "&limit=" + limit + "&tiporic=" + tiporic + "&query=" + escape(query) + "&page=" + page + "&ordine=" + ordine + "&action=elencoauto2",
		async: false,
		...
		<div class="container clearfix">  
			...
			<div class="sidebar">
				...
				<div class="widget-container widget_adv_filter">
					...
					<form action="ricerca2.php" class="side_form " onsubmit="return validation();">
